...
export function registerSyncCommand(program: Command): void {
  program
    ...
    .action(
      async (
        skillName: string,
        options: { to?: string; from?: string; dryRun: boolean; allowManagedModify?: boolean },
      ) => {
        try {
          if (!options.to && !options.from) {
            console.error(
              'Error: Either --to or --from option must be specified (e.g. --to claude:user or --from claude:user).',
            )
            process.exit(1)
          }

          const targetKeys = options.to ? (options.to.split(',').map((t) => t.trim()) as TargetKey[]) : undefined
