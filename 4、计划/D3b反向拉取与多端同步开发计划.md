...
1. **反向拉取（Pull）**：把 Agent 用户级目录中"自我进化"后的 Skill 拉回本地权威库 `library/skills/<skill-name>`，并重新计算 checksum、写回 registry。
2. **跨端中转（Cross-Agent Sync）**：通过本地权威库中转，把 A Agent 的 Skill 复制到 B Agent，例如 `claude:user → local → codex:user`。
...
- `src/sync/backup-local.ts` - 新增 `backupLocalLibraryBeforePull(root, backupDir, skillName, reason)`，本地库覆写前的备份索引，归类 `targetType: 'local'`。**不与 `backupBeforeSync`（目标端备份）合并**，与 D3a 文档"不要合并"原则一致。
...
- `src/core/registry.ts` - 复用；D3b **不修改** `localPath`（参见 §3.4）。
...
- `src/types/backup.ts` - `BackupItem.targetType` 扩展为 `'user' | 'project' | 'local'`，并补充 `targetAgent: 'local'`、`targetSkillPath: <localSkillDir>` 字段。
...
1. **处理 `pull` 项**：
   ...
   1.5 用新 metadata 重写 registry.skills[skillName]：`checksum / version / description / hasScripts / hasReferences / hasAssets / lastModified` 全部更新；`localPath` 保持原值；`syncedTargets` 不变（参见 §3.7）。
...
3. **跨端中转的两步回滚**：
   ...
   - 已写入的本地库从备份中恢复（仅跨端场景：`pull` 已成功 → 后续失败时，把 local 恢复到 pull 前快照）。
   - 备份目录保留以便人工排查。

### 3.4 `localPath` 字段语义

- 反向拉取**不修改** `registry.skills[skillName].localPath`：
  - `localPath` 是用户原始开发目录的引用，反向拉取只刷新本地库；用户开发目录与本地库的一致性由 D9 Watch Mode 负责。
  - 保持 `localPath` 不变能让反向拉取结果与 `import --force` 行为可区分：前者不破坏用户开发环境。

- 如果反向拉取源文件与 `localPath` 当前内容不同：
  - plan 中提示"本地开发目录 `localPath` 与拉回版本不一致"，**不修改** `localPath`。
  - 在 `ApplyResult.warnings: string[]` 报告这类提醒（types/plan.ts 同步扩展）。

### 3.5 本地库覆写前的备份

- 新增 `src/sync/backup-local.ts` 中的 `backupLocalLibraryBeforePull(root, backupDir, skillName, reason)`：
  - 内部复用 `backupSkillAndRegistry`（D1 既有）的物理备份逻辑，**不要合并函数**。
  - `BackupItem.targetType` 新增 `'local'`；`BackupItem.targetAgent = 'local'`；`BackupItem.targetSkillPath = <localSkillDir>`。
...
2. **`--from claude:user --to local`，hash 不同**：本地库覆写；deploy tag 不写本地库；registry.checksum 更新但 `localPath` 不变。
...
6. **`--from claude:user --to codex:user`，跨端中转**：plan 包含 `pull` + `create` 两步；apply 后本地库、codex 端都更新；registry.syncedTargets 反映 codex；备份目录出现两份（本地库一份、codex 端一份）。
7. **apply 中途失败回滚**：跨端场景下让 `create` 步骤失败，path guard 拒绝 → 本地库应被恢复到 pull 前快照；registry 应回滚到 apply 前。
...
11. **`localPath` 不变**：反向拉取后 `registry.skills[skillName].localPath` 等于 apply 前。
12. **`ApplyResult.warnings`**：拉回版本与 `localPath` 当前内容不一致时返回警告字符串。
...
- **`localPath` 与本地库漂移**：D3b 故意不更新 `localPath`，但用户开发目录与本地库会出现版本差；UI 需展示"本地开发目录与本地库版本差"提醒（依赖 `ApplyResult.warnings`）。
...
- **`backupBeforeSync` vs `backupSkillAndRegistry` vs `backupLocalLibraryBeforePull`**：三者职责清晰——`backupBeforeSync` 仅作用于目标端 user 目录；`backupSkillAndRegistry` 作用于本地库导入；`backupLocalLibraryBeforePull` 作用于反向拉取覆写本地库。**不要合并**，与 D3a 文档"不要合并"原则一致。
