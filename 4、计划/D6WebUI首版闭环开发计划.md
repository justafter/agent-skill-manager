...
### 2.12 项目工作区独立详情页 `/projects/:id`

D7 实施时为避免 `ProjectSpacePage` 上的"管理工作区"按钮变成大型 Modal（其内部同时承载 Skill 注入 / AI 规则同步 / 项目状态总览，体积膨胀），将工作区拆为独立子路由。

- 入口：项目卡片上的 `管理工作区` 按钮改为 `react-router-dom` 的 `useNavigate` 跳转到 `/projects/:id`。
- 路由：`web/src/pages/ProjectWorkspacePage.tsx`，`App.tsx` 增加 `<Route path="/projects/:id" element={<ProjectWorkspacePage />} />`；`/projects` 列表页保留 `ProjectSpacePage`。
- 数据源：复用既有 API（不新增后端接口）：
  - `GET /api/projects` → 取项目基础信息与扫描摘要；
  - `GET /api/skills` → Skill 下拉与导入目录；
  - `POST /api/projects/:id/inject/plan`、`POST /api/projects/:id/inject/apply` → 项目级 Skill 注入；
  - `GET /api/projects/:id/rules/diff?agent=...`、`POST /api/projects/:id/rules/sync` → 规则 diff / 推送 / 拉取；
  - **`POST /api/projects/:id/scan`**（2026-07-08 新增）→ 仅对单个项目触发重新扫描，返回 `{ projectId, skillDirs, ruleFiles }`；详见 §2.12.2。
- 页面结构（由上至下）：
  1. 顶部操作条：`返回项目空间` 按钮（跳 `/projects`）、`重新扫描` 按钮（可选，存在则启用）、项目名称 + ID + 启用 Agent tag。
  2. **项目基本信息卡**：项目 ID、物理路径、启用 Agent 列表、最近扫描时间、Skill 目录 / Rule 文件计数。
  3. **项目级 Skill 与 Rule 文件状态总览**：两组折叠区，分别列出 `project.scan.skillDirs` 与 `project.scan.ruleFiles`，每行 tag 等宽（`min-width: 90px; text-align: center`）+ 绝对路径 `flex: 1; min-width: 0`，与 Skill 卡片路径行同源（§2.8 / §2.11）。
  4. **技能注入区**：原 Modal 的 Skill tab 内容（选择 Skill、选择 Agent、生成注入计划、确认 apply）。
  5. **AI 规则同步区**：原 Modal 的 Rules tab 内容（选择 Agent、查看 Diff、推送 ↑ / 拉取 ↓）。
- 把现有 `ProjectSpacePage` 中的「管理工作区」Modal（含 tabs、injectPlanResult、selectedProject 等约 200 行 state 与逻辑）整体删除，迁出到独立页面；`ProjectSpacePage` 仅保留列表 / 添加 / 移除项目能力。
- 路由层 `ProjectWorkspacePage` 通过 `useParams<{ id: string }>()` 解析 id；若 `id` 不在 `projects` 列表内，显示「项目不存在」占位并提供返回按钮。
- 重新扫描走 §2.12.2 单项目扫描接口，**不再依赖** `refetchProjects()`（避免每次重新扫描都把所有项目都扫一遍）。

#### 2.12.2 项目级单项目扫描接口 `POST /api/projects/:id/scan`

（2026-07-08 补）`ProjectWorkspacePage.handleRescan` 不再用 `refetchProjects()` 触发"全量扫描全部项目"，改为精准调用 `POST /api/projects/:id/scan`：

- **后端**：在 `src/server/routes/projects.ts` 增加路由；行为：
  1. 加载 config，按 `:id` 查找项目；找不到抛 `NOT_FOUND`。
  2. 调用 `scanProject(p)`；项目目录不存在 / 扫描抛错时返回空 `{ projectId, skillDirs: [], ruleFiles: [] }`，**不抛 500**（与现有 `GET /api/projects` 对单项目失败的处理一致）。
  3. 返回 `{ projectId, skillDirs, ruleFiles, scannedAt: <ISO 字符串> }`。
- **前端**：`handleRescan` 改为调 `apiPost('/api/projects/${id}/scan')`，把响应里的 `skillDirs` / `ruleFiles` / `scannedAt` 写入 `ProjectWorkspacePage` 内部 state，**优先于** list 接口中的 `project.scan`，并在 UI 顶部提示「最后扫描时间：…」。`refetchProjects()` 仍作为兜底（其它卡片列表保持新鲜）；但页面上展示的项目级 Skill/Rule 路径以本次单项目扫描结果为准。

#### 2.12.1 项目卡片"管理工作区"说明引导

为了降低独立页面认知成本，项目卡片在 `管理工作区` 按钮旁增加一个轻量帮助图标 `<ProjectWorkspaceHint>`（首版可直接内联到 `ProjectSpacePage`，不需要新建组件文件）：

- 图标：`ⓘ` 或 `?`（HTML 实体 `&#9432;`），与按钮同行、字号 14px、颜色 `#2563eb`、`cursor: help`。
- 行为：`hover` 显示原生 `title` 提示 + 点击展开一个浮层（可用 `<details>`/`<summary>`，或 `<button onClick>` 切换 `showHint`）。浮层内容为以下说明文案（不写文件 / 不联网，纯前端）：

  > **工作区页能做什么？**
  >
  > - 查看本项目下已检测到的 Skill 目录与 Rule 文件的绝对路径；
  > - 选择本地库中的 Skill，按 Agent 类型（CLAUDE / CODEX / GEMINI）注入到项目级目录；
  > - 同步 `CLAUDE.md` / `AGENTS.md` / `GEMINI.md` 与本地模板的差异；
  > - 重新扫描项目目录以刷新检测结果。

- 不影响点击 `管理工作区` 按钮的导航行为；浮层默认收起。
- 不引入新依赖。
